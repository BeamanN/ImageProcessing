...
imatrix* add(imatrix* m1, imatrix* m2){
    ...
    {
        imatrix* m3 = init_blank_rgb_image(m1->width,m1->height,CHANNEL_NUM);
        for(a = 0; a < m1->height; a++)
        {
            for(b = 0; b < m1->width; b++)
            {
                m3->r[a][b] = m1->r[a][b] + m2->r[a][b];
                m3->g[a][b] = m1->g[a][b] + m2->g[a][b];
                m3->b[a][b] = m1->b[a][b] + m2->b[a][b];
            }
        }
        m3->write_rgb_to_image(m3);
        return m3;
    }
    else{
        return NULL;
...
imatrix* subtract(imatrix* m1, imatrix* m2){
    ...
    {
        imatrix* m3 = init_blank_rgb_image(m1->width,m1->height,CHANNEL_NUM);
        for(x = 0; x < m1->height; x++)
        {
            for(z = 0; z < m1->width; z++)
            {
                m3->r[x][z] = m1->r[x][z] - m2->r[x][z];
                m3->g[x][z] = m1->g[x][z] - m2->g[x][z];
                m3->b[x][z] = m1->b[x][z] - m2->b[x][z];
            }
        }
        m3->write_rgb_to_image(m3);
        return m3;
...
imatrix* multiply(imatrix* m1, imatrix* m2){
    ...
    if((width1 == width2) && (height1 == height2))
    {
        for(jj = 0; jj < height1; ++jj)
        {
            ...
            {
                res->r[jj][ii]=m1->r[jj][ii] * m2->r[jj][ii];
                res->g[jj][ii]=m1->g[jj][ii] * m2->g[jj][ii];
                res->b[jj][ii]=m1->b[jj][ii] * m2->b[jj][ii];
            }
        }
...
imatrix* scale(imatrix* this, int width, int height, float alpha){
    ...
    for(x1=0; x1 < height; ++x1)
    {
        ...
        {
            this->r[x1][y1] = this->r[x1][y1]*alpha;
            this->g[x1][y1] = this->r[x1][y1]*alpha;
            this->b[x1][y1] = this->r[x1][y1]*alpha;
        }
    }
    this->write_rgb_to_image;
